...
namespace Plus.HabboHotel.Roleplay.Combat
{
    public class BombCombat
    {
        public static bool CanExecuteAttack(GameClient Session, GameClient TargetSession)
        {
            ...
            if (Target.IsAsleep)
            {
                Session.SendWhisper("Non puoi eseguire questo comando perché è assente!");
                return false;
            }
        ...
        public static void ExecuteAttack(GameClient Session, GameClient TargetSession)
        {
            ...
            if (TargetSession.GetRoleplay().CurHealth <= 0)
            {
               ...
               Session.GetHabbo().GetRoomUser().LastBubble = 3;
                ...
                {*/
                    ...
                    RoleplayManager.Shout(Session, "*Throws my Grenade at " + TargetSession.GetHabbo().UserName + ", causing " + Damage + " damage and killing them ! [-4 Energy, +" + score + " PTS]*");
                /*}
                if (Session.GetRoleplay().LastKilled == TargetSession.GetHabbo().UserName)
                {
            ...
            else
            {
               ...
               TargetSession.GetHabbo().GetRoomUser().LastBubble = 3;
                ...
                RoleplayManager.Shout(Session, "*Lancia una granta a " + TargetSession.GetHabbo().UserName + ", causando " + Damage + " di danno [-4 Energia]*", 3);
                if (TargetSession.GetRoleplay().Armor >= 1)
                {
                    Misc.RoleplayManager.commandShout(TargetSession, "*[" + TargetSession.GetRoleplay().Armor + "LP Rimanenti!]*");
                }
                else
                {
