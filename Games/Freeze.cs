...
namespace Plus.HabboHotel.Rooms.Games
{
    internal class Freeze
    {
            ...
            foreach (
                var user in
                    _freezeTiles.Values.Select(
                        ...
                        .SelectMany(users => users.Where(user => user != null && user.Team == Team.none))) _room.GetGameMap().TeleportToItem(user, ExitTeleport);
        }
                ...
                var roomItem in
                    _freezeBlocks.Values.Where(
                        roomItem =>
                            ...
                            roomItem.FreezePowerUp != FreezePowerUp.None)) PickUpPowerUp(roomItem, user);
        }
        ...
        internal void AddFreezeTile(RoomItem item) { _freezeTiles.AddOrUpdate(item.Id, item, (k, v) => item); }
        ...
        internal void AddFreezeBlock(RoomItem item) { _freezeBlocks.AddOrUpdate(item.Id, item, (k, v) => item); }
        ...
        internal void FreezeStart() { throw new NotImplementedException(); }

        internal void FreezeEnd() { throw new NotImplementedException(); }
        ...
        private void FreezeUser(RoomUser user)
        {
            ...
            if (user.FreezeLives <= 0)
            {
                ...
                if (!managerForFreeze.BlueTeam.Any() && !managerForFreeze.RedTeam.Any() &&
                    !managerForFreeze.GreenTeam.Any() && managerForFreeze.YellowTeam.Any()) StopGame();
                ...
                {
                    if (managerForFreeze.BlueTeam.Any() || managerForFreeze.RedTeam.Any() ||
                        !managerForFreeze.GreenTeam.Any() || managerForFreeze.YellowTeam.Any()) return;
                    StopGame();
                }
            }
            else
        ...
        private List<RoomItem> GetItemsForSquare(Point point) { return _room.GetGameMap().GetCoordinatedItems(point); }
    }
}