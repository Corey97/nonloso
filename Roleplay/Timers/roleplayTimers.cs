                            ...
                            if (Session.GetRoleplay().Dead)
                            {
                                ...
                                if (Session.GetRoleplay().DeadTimer > 0)
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        if (Session.GetRoleplay().DeadTimer > 0)
                                        {
                                            Session.SendWhisper("Devi aspettare ancora un " + Session.GetRoleplay().DeadTimer + " minuto prima di essere dimesso, oppure prova a farti curare dal medico!");
                                        }
                                ...
                                else
                                {
                                    ...
                                    RoleplayManager.Shout(Session, "*Riprende conoscenza*", 0);
                    ...
                    case "jail":
                        {
                            if (Session.GetRoleplay().Jailed)
                            {
                                ...
                                else
                                {
                                    ...
                                    RUser.ClearMovement();
                                    RoleplayManager.Shout(Session, "*Finishes their time in jail*", 0);
                    ...
                    case "workout":
                        {
                            if (Session.GetRoleplay().WorkingOut)
                            {
                                ...
                                if (Session.GetRoleplay().RequestedTaxi || !Session.GetHabbo().CurrentRoom.RoomData.Description.Contains("GYM") || !Session.GetRoleplay().NearItem("sf_roller", 0))
                                {
                                    ...
                                    RoleplayManager.Shout(Session, "*Smette di allenarsi*");
                                ...
                                {
                                    Session.GetRoleplay().WorkingOut = false;
                                    RoleplayManager.Shout(Session, "*Si addormenta e sbatte la testa sul tasto di spegnimento, smettendo di allenarsi*");
                                    StoppedWorkingOut = true;
                                }
                                ...
                                if (!StoppedWorkingOut)
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        {
                                            Session.SendWhisper("Non puoi superare il livello 20.");
                                            Session.GetRoleplay().WorkingOut = false;
                                            RoleplayManager.Shout(Session, "*Smette di allenarsi*");
                                            StoppedWorkingOut = true;
                                        }
                                        else
                                        {
                                            ...
                                            RoleplayManager.Shout(Session, "*Si sente più forte*");
                                            Session.SendWhisper("Hai guadagnato +1 Forza. La tua Forza attuale: " + Session.GetRoleplay().Strength);
                                            Session.GetRoleplay().WorkoutSeconds = 40;
                                        }
                                    }
                                }
                    ...
                    case "weightlift":
                        {
                            if (Session.GetRoleplay().WeightLifting)
                            {
                                ...
                                {
                                    Session.GetRoleplay().WeightLifting = false;
                                    RoleplayManager.Shout(Session, "*Smette di allenarsi, e si addormenta sulla panca*");
                                    StoppedWeightLifting = true;
                                }
                                ...
                                if (!StoppedWeightLifting)
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        {
                                            Session.SendWhisper("Non puoi andare oltre il livello 10.");
                                            Session.GetRoleplay().WeightLifting = false;
                                            RoleplayManager.Shout(Session, "*Smette di allenarsi*");
                                            StoppedWeightLifting = true;
                                        }
                                        else
                                        {
                                            ...
                                            RoleplayManager.Shout(Session, "*Si sente più in forma*");
                                            Session.SendWhisper("Hai guadagnato +1 Costituzione. La tua Costituzione attuale: " + Session.GetRoleplay().Constitution);
                                            Session.SendWhisper("Hai guadagnato +1 Vita Massima. La tua Vita Massima attuale: " + Session.GetRoleplay().MaxHealth);
                                            Session.GetRoleplay().WeightLiftSeconds = 40;
                                        }
                                    }
                                }
                    ...
                    case "rp":
                        {
                            ...
                            */
                            ...
                            if (Session.GetRoleplay().InShower)
                            {
                                if (Session.GetRoleplay().ShowerSeconds <= 0)
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        Session.GetRoleplay().InShower = false;
                                        RUser.MoveTo(RUser.X, RUser.Y - 1);
                                        Session.SendWhisper("Ora sei pulito!");
                                        Session.Shout("*Dopo una bella rinfrescata è pronto per uscire dalla doccia.*");
                                        Session.GetRoleplay().Shower.ExtraData = "0";
                                        Session.GetRoleplay().Shower.UpdateNeeded = true;
                                        Session.GetRoleplay().Shower.UpdateState();
                                        Session.GetRoleplay().InShower = false;
                            ...
                            if (Session.GetRoleplay().Working)
                            {
                                ...
                                {
                                    RoleplayManager.Shout(Session, "*Si leva la divisa, pronto a godersi la giornata.*");
                                    Session.GetRoleplay().StopWork();
                                }
