...
namespace Plus.Messages.Handlers
{
    ...
    partial class GameClientMessageHandler
    {
        ...
        internal void ChangeLook()
        {
            ...
            if (!Session.GetHabbo().CurrentRoom.RoomData.Description.Contains("CLOTHING_STORE") && !RoleplayManager.BypassRights(Session))
            {
                Session.SendWhisper("Devi stare in una stanza apposita: Bagni, Spogliatoi, Negozio di vestiti.");
                return;
            }

            if (Session.GetRoleplay().Working)
            {
                Session.SendWhisper("Non puoi cambiarti d'abito mentre sei a lavoro.");
                return;
            }

            if (Session.GetHabbo().Credits < 0)
            {
                Session.SendWhisper("Non hai 0 crediti..");
                return;
            }
        ...
        internal void ChangeMotto()
        {
            ...
            {
                Session.SendWhisper("Non puoi cambiare la tua missione!");
                return;
            }
