...
namespace Plus.HabboHotel.Catalogs
{
    ...
    class Catalog
    {
        ...
        {
            return petName.Length >= 3 && petName.Length <= 15 && Plus.IsValidAlphaNumeric(petName);
        }
        ...
        {
            {
                var pet = new Pet(404u, userId, 0u, name, (uint)type, Race, Color, 0, 100, 100, 0,
                    Plus.GetUnixTimeStamp(), 0, 0, 0.0, false, 0, 0, -1, Rarity,
        ...
        internal static Pet GeneratePetFromRow(DataRow Row, DataRow mRow)
        {
            ...
            return new Pet(Convert.ToUInt32(Row["id"]), Convert.ToUInt32(Row["user_id"]),
                ...
                Plus.UnixToDateTime((int)mRow["lasthealth_stamp"]),
                Plus.UnixToDateTime((int)mRow["untilgrown_stamp"]), moplaBreed);
        }
        ...
        internal CatalogItem GetItemFromOffer(int offerId)
        {
            ...
            return result ?? (Plus.GetGame().GetCatalog().GetItem(Convert.ToUInt32(offerId)));
        }
        ...
        internal void Initialize(IQueryAdapter dbClient)
        {
            ...
            if (table3 != null)
                foreach (DataRow dataRow in table3.Rows)
                {
                    ...
                    var item = Plus.GetGame().GetItemManager().GetItem(id);
                    if (item == null)
                        continue;
            ...
            uint @group)
        {
            ...
            foreach (var baseItem in item.Items.Keys)
            {
                if (isGift)
                {
                    ...
                    {
                        session.SendNotif(Plus.GetLanguage().GetVar("user_send_gift"));
                        return;
                    }
                ...
                {
                    session.SendNotif(Plus.GetLanguage().GetVar("user_send_gift_effect"));
                    return;
                }
                    ...
                    case Interaction.RoomEffect:
                    {
                        ...
                        try
                        {
                            number = string.IsNullOrEmpty(extraData)
                                ? 0.0
                                : double.Parse(extraData, Plus.CultureInfo);
                        }
                ...
                if (isGift)
                {
                    ...
                    var clientByUserId =
                    Plus.GetGame().GetClientManager().GetClientByUserId(toUserId);
            ...
            int limtot, string songCode)
        {
...

            {
                                        ...
                                        switch (interactionType)
                                        {
                                            ...
                                            case Interaction.GuildForum:
                                                ...
                                                if (group != null)
                                                {
                                                    ...
                                                    {
                                                        session.SendNotif(Plus.GetLanguage().GetVar("user_group_owner_error"));
                                                    }
                                                }
                ...
                else if (a == 'r')
                {
                    ...
                    {
                        var bot = CreateBot(session.GetHabbo().Id, "Jacob",
                            "hr-3020-34.hd-3091-2.ch-225-92.lg-3058-100.sh-3089-1338.ca-3084-78-108.wa-2005",
                            "Satisfies your thirst, and does everything the casual bot can, too!", "m", true);
                    ...
                    {
                        var bot2 = CreateBot(session.GetHabbo().Id, "Melissa",
                            "hr-9534-39.hd-600-1.ch-819-92.lg-3058-64.sh-3064-110.wa-2005",
                            "Can talk, walk, dance and dress up", "f", false);
        ...
        {
            var level = 1u;
            if (Plus.GetRandomNumber(1, 2000) == 2000)
                level = 5u;
            else if (Plus.GetRandomNumber(1, 200) == 200)
                level = 4u;
            else if (Plus.GetRandomNumber(1, 40) == 40)
                level = 3u;
            else if (Plus.GetRandomNumber(1, 4) == 4)
                level = 2u;
            var ecotronRewardsForLevel = GetEcotronRewardsForLevel(level);
            return
                ecotronRewardsForLevel[Plus.GetRandomNumber(0, (ecotronRewardsForLevel.Count - 1))];
        }
