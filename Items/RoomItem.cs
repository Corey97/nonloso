...
using Plus.HabboHotel.Rooms.Wired.Handlers;
using Plus.HabboHotel.SoundMachine;
using Plus.Messages;
using Plus.Messages.Parsers;
using Plus.Util;
using Plus.Database.Manager.Database.Session_Details.Interfaces;
//using Plus.HabboHotel.Roleplay.Jobs.Farming.Objects;
...
namespace Plus.HabboHotel.Items
{
    ...
    public class RoomItem : IEquatable<RoomItem>
    {
        ...
        internal bool OnNPAUsing = false;
                          ...
                          Room pRoom, uint userid, uint eGroup, int flatId, string songCode, bool isBuilder)
        {
            ...
            _mBaseItem = Plus.GetGame().GetItemManager().GetItem(baseItem);
            _mRoom = pRoom;
        ...
        internal double Height
        {
            get
            {
                ...
                {
#pragma warning disable CS0436 // Il tipo è in conflitto con il tipo importato
...
#pragma warning restore CS0436 // Il tipo è in conflitto con il tipo importato
        ...
        internal double TotalHeight
        {
            get
            {
                try
                {
                    if (this.GetBaseItem().StackMultipler && !string.IsNullOrWhiteSpace(ExtraData))
                    {
                        int num = Convert.ToInt32(ExtraData);
                        return Z + Convert.ToDouble(GetBaseItem().ToggleHeight[num]);
                    }
                    if (this.GetBaseItem().VariableHeight != "")
                    {
                        string[] Heights = GetBaseItem().VariableHeight.Split(';');
                        if (ExtraData == "")
                        {
                            ExtraData = "0";
                        }
                        if (ExtraData == "0")
                        {
                            return (Z + double.Parse(Heights[0]));
                        }
                        else
                        {
                            return (Z + double.Parse(Heights[int.Parse(ExtraData)]));
                        }
                    }
                    else
                    {
...
#pragma warning restore CS0436 // Il tipo è in conflitto con il tipo importato
        ...
        internal IFurniInteractor Interactor
        {
            get
            {
                ...
                {
                    case Interaction.trash_cans:
                        return new InteractorTrashCans();

                    case Interaction.rp:
                        return new InteractorRP();

                    case Interaction.slotmachine:
                        return new InteractorSlotMachine();

                    case Interaction.minefield:
                        return new InteractorMinefield();
        ...
        internal void ProcessUpdates()
        {
            ...
            switch (interactionType)
            {
                    ...
                    if (ExtraData == "1")
                    {
                        ...
                        if (roomUser2 != null)
                        {
                            ...
                            if (GetBaseItem().VendingIds.Count > 0)
                            {
                                ...
                                if (Substances.SubstanceData.ContainsKey(GetBaseItem().Name.ToLower()))
                                {

                                    if (!roomUser2.GetClient().GetRoleplay().MultiCoolDown.ContainsKey("vender_cooldown"))
                                    {
                                        roomUser2.GetClient().GetRoleplay().MultiCoolDown.Add("vender_cooldown", 0);
                                    }
                                    if (roomUser2.GetClient().GetRoleplay().MultiCoolDown["vender_cooldown"] > 0)
                                    {
                                        roomUser2.GetClient().SendWhisper("Cooldown [" + roomUser2.GetClient().GetRoleplay().MultiCoolDown["vender_cooldown"] + "/5]");
                                        InteractingUser = 0;
                                        this.ExtraData = "0";
                                        return;
                                    }

                                    if (roomUser2.GetClient().GetRoleplay().inColourWars == false)
                                    {
                                        if (Substances.SubstanceData[GetBaseItem().Name.ToLower()].Item_Price > roomUser2.GetClient().GetHabbo().Credits)
                                        {
                                            roomUser2.GetClient().SendWhisper("You need at least $" + Substances.SubstanceData[GetBaseItem().Name.ToLower()].Item_Price + " to use this!");
                                            InteractingUser = 0;
                                            this.ExtraData = "0";
                                            return;
                                        }
                                    ...
                                    if (roomUser2.GetClient().GetRoleplay().inColourWars == false)
                                    {
                                        RoleplayManager.GiveMoney(roomUser2.GetClient(), -Substances.SubstanceData[GetBaseItem().Name.ToLower()].Item_Price);
                                    }
                                    if (Substances.SubstanceData[GetBaseItem().Name.ToLower()].Speech.Length > 2)
                                    {
                                        string FilterSpeech1 = Substances.SubstanceData[GetBaseItem().Name.ToLower()].Speech.Replace("%price%", "" + Substances.SubstanceData[GetBaseItem().Name.ToLower()].Item_Price);
                                        string FilterSpeech2 = FilterSpeech1.Replace("%extra%", "[+" + Substances.SubstanceData[GetBaseItem().Name.ToLower()].Energy + " Energy][-$" + Substances.SubstanceData[GetBaseItem().Name.ToLower()].Item_Price + "]");
                                        RoleplayManager.Shout(roomUser2.GetClient(), FilterSpeech2, 0);
                                        roomUser2.GetClient().GetRoleplay().Energy += Substances.SubstanceData[GetBaseItem().Name].Energy;
                                        roomUser2.GetClient().GetRoleplay().CurHealth += Substances.SubstanceData[GetBaseItem().Name].Health;
                                        roomUser2.GetClient().GetRoleplay().Hunger += Substances.SubstanceData[GetBaseItem().Name].Hunger;
                                    }

                                    roomUser2.GetClient().GetRoleplay().MultiCoolDown["vender_cooldown"] = 5;
                                    roomUser2.GetClient().GetRoleplay().CheckingMultiCooldown = true;
                ...
                case Interaction.Dice:
                    try
                    {
                        RoomUser RigUser = GetRoom().GetRoomUserManager().GetRoomUserByHabbo(InteractingUser);
                        if (RigUser.GetClient().GetHabbo().RollRig > 0)
                        {
                            ExtraData = RigUser.GetClient().GetHabbo().RollRig.ToString();
                            RigUser.GetClient().GetHabbo().RollRig = 0;
                        }
                        else
                        {
                ...
                case Interaction.QuickTeleport:
                    {
                        ...
                        if (InteractingUser > 0)
                        {
                            ...
                            if (roomUserByHabbo2.Coordinate == Coordinate)
                            {
                                ...
                                if (TeleHandler.IsTeleLinked(Id, _mRoom))
                                {
                                    ...
                                    if (teleRoomId == RoomId)
                                    {
                                        ...
                                        if (item2 == null) roomUserByHabbo2.UnlockWalking();
                                        else
                                        {
                            ...
                            else if (roomUserByHabbo2.Coordinate == SquareInFront)
                            {
                                ...
                                if (roomUserByHabbo2.IsWalking &&
                                    (roomUserByHabbo2.GoalX != X || roomUserByHabbo2.GoalY != Y)) roomUserByHabbo2.ClearMovement();
                                roomUserByHabbo2.SetRot(PathFinder.CalculateRotation(roomUserByHabbo2.X, roomUserByHabbo2.Y,
                                    X, Y));
                                roomUserByHabbo2.UnlockWalking();
                        ...
                        if (InteractingUser2 > 0u)
                        {
                            var roomUserByHabbo3 = GetRoom().GetRoomUserManager().GetRoomUserByHabbo(InteractingUser2);
                ...
                case Interaction.WearItem:
                    {
                        ...
                        var clientByUserId = Plus.GetGame().GetClientManager().GetClientByUserId(InteractingUser);
                        {
        ...
        internal void UpdateState(bool inDb, bool inRoom)
        {
            ...
            if (IsFloorItem)
            {
                ...
                switch (GetBaseItem().InteractionType)
                {
                    ...
                    case Interaction.CrackableEgg:
                        Plus.GetGame().GetCrackableEggHandler().GetServerMessage(serverMessage, this);
                        break;
            ...
            if (IsFloorItem)
            {
                ...
                switch (GetBaseItem().InteractionType)
                {
                    ...
                    case Interaction.GuildForum:
                        {
                            var group2 = Plus.GetGame().GetGroupManager().GetGroup(GroupId);
                            if (group2 == null)
                            {
                            ...
                            else
                            {
                                ...
                                message.AppendString(Plus.GetGame()
                                    .GetGroupManager()
                    ...
                    case Interaction.Gift:
                        {
                            ...
                            try
                            {
                                ...
                                showGiver = Plus.EnumToBool(split[4]);
                                giverName = split[5];
                                giverLook = split[6];
                                product = split[7];
                            }
                    ...
                    case Interaction.CrackableEgg:
                        message.AppendInteger(0);
                        Plus.GetGame().GetCrackableEggHandler().GetServerMessage(message, this);
                        break;
        ...
        {
            return _mBaseItem ?? (_mBaseItem = Plus.GetGame().GetItemManager().GetItem(BaseItem));
        }
