...
namespace Plus.HabboHotel.Navigators
{
    /// <summary>
    ...
    internal class SearchResultList
    {
        ...
        internal static void SerializePromotionsResultListFlatcats(int flatCatId, bool direct, ServerMessage message)
        {
            ...
            try
            {
                var rooms = Plus.GetGame().GetRoomManager().GetEventRooms();
                Plus.GetGame()
                    .GetNavigator()
                    .SerializeNavigatorPopularRoomsNews(ref message, rooms, flatCatId, direct);
            }
        ...
        {;
                ...
                !staticId.StartsWith("category__") && staticId != "official_view")
            {
                ...
                message.AppendBool(staticId != "my" && staticId != "official-root"); // collapsed
            ...
            switch (staticId)
            {
                case "hotel_view":
                {
                    ...
                    foreach (FlatCat flat in Plus.GetGame().GetNavigator().PrivateCategories.Values) SerializeSearchResultListFlatcats(flat.Id, false, message);
                    break;
                }
                case "myworld_view":
                {
                ...
                case "roomads_view":
                {
                    foreach (FlatCat flat in Plus.GetGame().GetNavigator().PrivateCategories.Values) SerializePromotionsResultListFlatcats(flat.Id, false, message);
                ...
                case "official-root":
                {
                    message.AppendServerMessage(Plus.GetGame().GetNavigator().NewPublicRooms);
                    break;
                }
                case "staffpicks":
                {
                    message.AppendServerMessage(Plus.GetGame().GetNavigator().NewStaffPicks);
                    break;
                }
                case "my":
                {
                        ...
                        var data in
                            session.GetHabbo()
                                .FavoriteRooms.Select(
                                    dataId => Plus.GetGame().GetRoomManager().GenerateRoomData(dataId))
                ...
                case "popular":
                {
                    try
                    {
                        rooms = Plus.GetGame().GetRoomManager().GetActiveRooms();
                        if (rooms == null)
                        {
                            message.AppendInteger(0);
                            break;
                ...
                case "top_promotions":
                {
                    try
                    {
                        rooms = Plus.GetGame().GetRoomManager().GetEventRooms();
                        message.AppendInteger(rooms.Length);
                        foreach (var room in rooms) room.Key.Serialize(message, false);
                    }
                    catch
                ...
                case "my_groups":
                {
                    ...
                    foreach (var data in from xGroupId in session.GetHabbo().MyGroups
                                         select Plus.GetGame().GetGroupManager().GetGroup(xGroupId)
                                         into xGroup
                                         where xGroup != null
                                         select Plus.GetGame().GetRoomManager().GenerateRoomData(xGroup.RoomId)
                                         into data
                                         where data != null
                                         select data)
                    {
                ...
                case "history":
                {
                    ...
                    foreach (var roomData in session.GetHabbo()
                        .RecentlyVisitedRooms.Select(
                            roomId => Plus.GetGame().GetRoomManager().GenerateRoomData(roomId)).Where(roomData => roomData != null))
                    {
        ...
        internal static void SerializeSearches(string searchQuery, ServerMessage message, GameClient session)
        {
            ...
            if (!containsOwner)
            {
                ...
                try
                {
                    if (Plus.GetGame().GetRoomManager().GetActiveRooms() != null &&
                        Plus.GetGame().GetRoomManager().GetActiveRooms().Any())
                        initForeach = true;
                }
                catch
                {
                ...
                if (initForeach)
                {
                    foreach (var rms in Plus.GetGame().GetRoomManager().GetActiveRooms())
                    {
                    ...
                    foreach (
                        var rData in
                            dTable.Rows.Cast<DataRow>()
                                .Select(
                                    row =>
                                        Plus.GetGame().GetRoomManager().FetchRoomData(Convert.ToUInt32(row["id"]), row))
