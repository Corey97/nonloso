...
namespace Plus.Messages.Handlers
{
    partial class GameClientMessageHandler
    {
        ...
        internal void PetBreedResult()
        {
            ...
            switch (item.GetBaseItem().InteractionType)
            {
                case Interaction.BreedingTerrier:
                    ...
                    Plus.GetGame().GetAchievementManager().ProgressUserAchievement(Session, "ACH_TerrierBreeder", 1);
                    break;

                case Interaction.BreedingBear:
                    if (room.GetRoomItemHandler().BreedingBear.ContainsKey(item.Id))
                        room.GetRoomItemHandler().BreedingBear.Remove(item.Id);
                    Plus.GetGame().GetAchievementManager().ProgressUserAchievement(Session, "ACH_BearBreeder", 1);
                    break;
            }
        ...
        internal void PlaceItem()
        {
            ...
            {
                var room = Plus.GetGame().GetRoomManager().GetRoom(Session.GetHabbo().CurrentRoomId);
                if (room == null || Plus.GetDbConfig().DbData["placing_enabled"] != "1")
                    return;
                                ...
                                case Interaction.Dimmer:
                                    {
                                        if (room.MoodlightData != null &&
                                            ...
                                            this.Session.SendNotif(Plus.GetLanguage().GetVar("room_moodlight_one_allowed"));
                                        goto PlaceWall;
                                    }
                                default:
                                    {
                                ...
                                case Interaction.Toner:
                                    {
                                        ...
                                        {
                                            this.Session.SendNotif(Plus.GetLanguage().GetVar("room_toner_one_allowed"));
                                            return;
                                        }
                                        goto PlaceFloor;
                                    }
            ...
            PlaceFloor:
                if (room.CheckRights(Session))
                    Plus.GetGame().GetQuestManager().ProgressUserQuest(Session, QuestType.FurniPlace, 0u);
            ...
            {
                Session.SendMessage(StaticMessage.ErrorCantSetItem);
                Writer.Writer.LogException("Error in PlaceItem():  " + e.ToString());
            }
        }
        ...
        internal void MoveItem()
        {
            ...
            if (x != item.X || y != item.Y)
                Plus.GetGame().GetQuestManager().ProgressUserQuest(Session, QuestType.FurniMove, 0u);
            if (rot != item.Rot)
                Plus.GetGame().GetQuestManager().ProgressUserQuest(Session, QuestType.FurniRotate, 0u);
            ...
            if (item.Z >= 0.1)
                Plus.GetGame().GetQuestManager().ProgressUserQuest(Session, QuestType.FurniStack, 0u);
        ...
        internal void OpenGift()
        {
            ...
            {
                Session.SendNotif(Plus.GetLanguage().GetVar("gift_one"));
                return;
            }
            var currentRoom = Session.GetHabbo().CurrentRoom;
            if (currentRoom == null)
            {
                Session.SendWhisper(Plus.GetLanguage().GetVar("gift_two"));
                return;
            }
            if (!currentRoom.CheckRights(Session, true, false))
            {
                Session.SendWhisper(Plus.GetLanguage().GetVar("gift_three"));
                return;
            }
            ...
            if (item == null)
            {
                Session.SendWhisper(Plus.GetLanguage().GetVar("gift_four"));
                return;
            }
            ...
            var queryReactor = Plus.GetDatabaseManager().GetQueryReactor();
            ...
            var item2 = Plus.GetGame().GetItemManager().GetItem(Convert.ToUInt32(row["item_id"]));
            if (item2 == null)
            {
                currentRoom.GetRoomItemHandler().RemoveFurniture(Session, item.Id, false);
                return;
            ...
            if (item2.Type.Equals('s'))
            {
                ...
                {
                    this.Session.SendNotif(Plus.GetLanguage().GetVar("error_creating_gift"));
                }
                else
                {
...

            {
                foreach (var current2 in room.MoodlightData.Presets)
                {
                    ...
                    Response.AppendInteger(
                        int.Parse(Plus.BoolToEnum(current2.BackgroundOnly)) + 1);
        ...
        internal void InitTrade()
        {
            ...
            {
                this.Session.SendNotif(Plus.GetLanguage().GetVar("room_trade_disabled"));
                return;
            }
            if (room.RoomData.TradeState == 1 && !room.CheckRights(Session))
            {
                this.Session.SendNotif(Plus.GetLanguage().GetVar("room_trade_disabled_no_rights"));
                return;
            }
           
            if (!Session.GetHabbo().CheckTrading())
                this.Session.SendNotif(Plus.GetLanguage().GetVar("room_trade_disabled_mod"));
        ...
        internal void RecycleItems()
        {
            if (!Session.GetHabbo().InRoom)
                return;
            var num = Request.GetInteger();
            if (num != Convert.ToUInt32(Plus.GetDbConfig().DbData["recycler.number_of_slots"]))
                return;
            var i = 0;

            {
                ...
                using (var queryreactor2 = Plus.GetDatabaseManager().GetQueryReactor())
                {
                    ...
                    queryreactor2.AddParameter("baseItem", Convert.ToUInt32(Plus.GetDbConfig().DbData["recycler.box_id"]));
        ...
        internal void RedeemExchangeFurni()
        {
            ...
            if (Plus.GetDbConfig().DbData["exchange_enabled"] != "1")
            {
                this.Session.SendNotif(Plus.GetLanguage().GetVar("bliep_wisselkoers_uitgeschakeld"));
                return;
            }
            var item = room.GetRoomItemHandler().GetItem(Request.GetUInteger());
            if (item == null) return;
            ...
            using (var queryReactor = Plus.GetDatabaseManager().GetQueryReactor()) queryReactor.RunFastQuery(string.Format("DELETE FROM items_rooms WHERE id={0} LIMIT 1;", item.Id));
        ...
        internal void TriggerLoveLock(RoomItem loveLock)
        {
            ...
            try
            {
                ...
                {
                    this.Session.SendNotif(Plus.GetLanguage().GetVar("lovelock_error_1"));
                    return;
                }
                if (roomUserOne.GetClient() == null || roomUserTwo.GetClient() == null)
                {
                    this.Session.SendNotif(Plus.GetLanguage().GetVar("lovelock_error_2"));
                    return;
                }
            ...
            {
                this.Session.SendNotif(Plus.GetLanguage().GetVar("lovelock_error_3"));
            }
        }
        ...
        internal void CompostMonsterplant()
        {
            ...
            if (room == null || !room.CheckRights(Session, true))
            {
                this.Session.SendNotif(Plus.GetLanguage().GetVar("monsterplant_error_1"));
                return;
            }
            ...
            {
                this.Session.SendNotif(Plus.GetLanguage().GetVar("monsterplant_error_2"));
                return;
            }
            if (pet.PetData.MoplaBreed.LiveState != MoplaState.Dead)
            {
                this.Session.SendNotif(Plus.GetLanguage().GetVar("monsterplant_error_3"));
                return;
            }
            var compostItem = Plus.GetGame().GetItemManager().GetItemByName("mnstr_compost");
            if (compostItem == null)
            {
                this.Session.SendNotif(Plus.GetLanguage().GetVar("monsterplant_error_4"));
                return;
            }
            var X = pet.X;
            var Y = pet.Y;
            ...
            using (var dbClient = Plus.GetDatabaseManager().GetQueryReactor())
            {
                ...
                if (!room.GetRoomItemHandler().SetFloorItem(Session, roomItem, X, Y, 0, true, false, true))
                {
                    ...
                    this.Session.SendNotif(Plus.GetLanguage().GetVar("monsterplant_error_5"));
                }
        ...
        internal void MovePet()
        {
            ...
            if (room == null || !room.CheckRights(Session))
            {
                this.Session.SendNotif(Plus.GetLanguage().GetVar("monsterplant_error_6"));
                return;
            }
            var petId = Request.GetUInteger();
            ...
            {
                this.Session.SendNotif(Plus.GetLanguage().GetVar("monsterplant_error_7"));
                return;
            }
            var x = Request.GetInteger();
            var y = Request.GetInteger();
            ...
            if (x != oldX && y != oldY)
                ...
                {
                    this.Session.SendNotif(Plus.GetLanguage().GetVar("monsterplant_error_8"));
                    return;
                }
        ...
        internal void PickUpPet(HabboHotel.GameClients.GameClient Session, uint petId, bool PickingUp = false)
        {
            var room =
                Plus.GetGame().GetRoomManager().GetRoom(Session.GetHabbo().CurrentRoomId);
            if (Session == null || Session.GetHabbo() == null ||
        ...
        internal void PickUpPet()
        {
            ...
            if(pet.BotData != null)
            {
                if(pet.BotData.OwnerId != Session.GetHabbo().Id)
                {
                    Session.SendWhisper("This is not your pet, stop trying to pick it up? If you're staff just use the :killpet command!");
                    return;
                }
            }
            ...
            Session.GetHabbo().GetRoomUser().MyPet = null;
            Session.GetRoleplay().UsingPet = false;
            HabboHotel.Roleplay.Misc.RoleplayManager.Shout(Session, "*Picks their pet up*");
...

            {
                if (pet.PetData.Type == 16)
                    Plus.GetGame()
                        ...
                        .ProgressUserAchievement(Session, "ACH_MonsterPlantTreater", 1, false);
                else
                {
        ...
        internal void RedeemVoucher()
        {
...

            {
                if (row != null)
                {
                    isValid = true;
                    using (
                        var queryreactor2 = Plus.GetDatabaseManager().GetQueryReactor())
                    {
        ...
        internal void MountOnPet()
        {
            ...
            {
                this.Session.SendNotif(Plus.GetLanguage().GetVar("horse_error_1"));
                return;
            }
            if (flag)
            {
            ...
            {
                this.Session.SendNotif(Plus.GetLanguage().GetVar("horse_error_2"));
                return;
            }
        ...
        internal void PlaceBot()
        {
            ...
            {
                this.Session.SendNotif(Plus.GetLanguage().GetVar("bot_error_1"));
                return;
            }
            using (var queryReactor = Plus.GetDatabaseManager().GetQueryReactor())
        ...
        internal void PlaceBuildersFurniture()
        {
            ...
            var item = Plus.GetGame().GetCatalog().GetItem(itemId);
        ...
        internal void EjectFurni()
        {
            ...
            if (item == null)
                return;
            var clientByUserId =
                Plus.GetGame().GetClientManager().GetClientByUserId(item.UserId);
        ...
        internal void UsePurchasableClothing()
        {
            ...
            var clothes = Plus.GetGame().GetClothingManager().GetClothesInFurni(item.GetBaseItem().Name);
        ...
        internal void GetCraftingRecipesAvailable()
        {
        }
        internal void CraftSecret()
        {

        }
        internal void SetCraftingRecipe()
        {

        }
        internal void ExecuteCraftingRecipe()
        {

        }
        internal void GetCraftingItem()
        {
