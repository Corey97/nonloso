            ...
            else
            {
                ...
                if (TargetSession.GetRoleplay().CurHealth <= 0)
                {
                    ...
                    if (!WeaponManager.WeaponsData.ContainsKey(Session.GetRoleplay().Equiped.ToLower()))
                    {
                        ...
                        Session.Shout("*Spara a: " + TargetSession.GetHabbo().UserName + ", causando " + Damage + " danno e mandarlo all'altro mondo! [-" + WeaponManager.WeaponsData[Session.GetRoleplay().Equiped.ToLower()].Need_Energy + " Energia, +" + score + " PNT]*");
                    ...
                    TargetSession.SendNotif("Sei stato ucciso da: " + Session.GetHabbo().UserName + ", verrai portato in ospedale.");
