...
{
    public class relaxTimer
    {
        Timer timer;
        GameClient Session;
        int timeLeft; // 10 seconds (milliseconds)

        public relaxTimer(GameClient Session)
        {
        ...
        public void timerDone(object info)
        {
            try
            {
                ...
                if (Session.GetRoleplay().Energy + PlusEnergy >= 100)
                {
                    ...
                    Session.SendWhisper("You are fully relaxed!");
                ...
                else
                {
                    ...
                    Session.GetRoleplay().SaveQuickStat("energy", "" + Session.GetRoleplay().Energy);
                    Session.SendWhisper("Your energy is now " + Session.GetRoleplay().Energy + "! Stay for more relaxing!");
                }
                if (Session.GetRoleplay().Energy != 100 && Session.GetRoleplay().Relaxing)
                {
