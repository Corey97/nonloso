...
namespace Plus.HabboHotel.Rooms
{
    ...
    class Gamemap
    {
        ...
        public Gamemap(Room room)
        {
            ...
            StaticModel = Plus.GetGame().GetRoomManager().GetModel(room.RoomData.ModelName, room.RoomId);
            if (StaticModel == null)
            ...
            GameClient client)
        {
            ...
            if (GuildGates.ContainsKey(square) && user.GetClient() != null && user.GetClient().GetHabbo() != null && user.GetClient().GetRoleplay() != null)
            {
                var roomItem = GuildGates[square];
                var guildId = roomItem.GroupId;
                if (guildId > 0 && user.GetClient().GetRoleplay().JobId == guildId)
                {
            ...
            if (GuildGates.ContainsKey(to))
            {
                ...
                if (guildId > 0)
                    if (user.GetClient().GetHabbo() != null &&
                        user.GetClient().GetRoleplay() != null &&
                        user.GetClient().GetRoleplay().JobId == guildId)
                    {
                        roomItem.ExtraData = "1";
                        roomItem.UpdateState();
                        return true;
        ...
        internal Point GetRandomValidWalkableSquare()
        {
            ...
            var randomNumber = Plus.GetRandomNumber(0, walkableSquares.Count);
            var i = 0;
        ...
        internal bool IsValidStep(RoomUser user, Vector2D From, Vector2D To, bool EndOfPath, bool Override)
        {
             ...
             * 3 = door
             * */
            if ((this.GameMap[To.X, To.Y] == 3 && !EndOfPath) ||
               (this.GameMap[To.X, To.Y] == 0 && !EndOfPath) || (this.GameMap[To.X, To.Y] == 2 && !EndOfPath))
            {
        ...
        /// <summary>
        /// Checks if the map is all good
        /// </summary>
        /// <param name="End">The end position</param>
        /// <returns>True if valid, false if not.</returns>
        public bool MapOk(Vector2D To)
        {
            if (To.X < this.GameMap.GetLength(0) && To.Y < this.GameMap.GetLength(1))
            {
                if ((this.GameMap[To.X, To.Y] != 0))
                {
                    return true;
                }
            }
        ...
        /// <summary>
        /// Checks if the square is bad
        /// </summary>
        /// <param name="End">The end position</param>
        /// <returns>True if valid, false if not.</returns>
        public bool squareBlocked(Vector2D To)
        {
            if (To.X < this.GameMap.GetLength(0) && To.Y < this.GameMap.GetLength(1))
            {
                if ((this.GameMap[To.X, To.Y] == 0))
        ...
        /// <summary>
        /// Gets an alternative position
        /// </summary>
        /// <param name="End">The end position</param>
        /// <returns>True if valid, false if not.</returns>
        public Vector2D squareBlockedAlternativeSquare(Vector2D To)
        {

            try
            {
                #region First resort 1 space

                if (To.X + 1 < this.GameMap.GetLength(0) && To.Y < this.GameMap.GetLength(1))
                {
                    if ((this.GameMap[To.X + 1, To.Y] == 1))
                    {
                        return new Vector2D(To.X + 1, To.Y);
                    }
                }
                ...
                if (To.X < this.GameMap.GetLength(0) && To.Y + 1 < this.GameMap.GetLength(1))
                {
                    if ((this.GameMap[To.X, To.Y + 1] == 1))
                    {
                        return new Vector2D(To.X, To.Y + 1);
                    }
                }
                ...
                if (To.X + 1 < this.GameMap.GetLength(0) && To.Y - 1 < this.GameMap.GetLength(1))
                {
                    if ((this.GameMap[To.X + 1, To.Y - 1] == 1))
                    {
                        return new Vector2D(To.X + 1, To.Y - 1);
                    }
                }
                ...
                #region Last Resort 2 space
                // 2 SPACES  LAST RESORT
                if (To.X < this.GameMap.GetLength(0) && To.Y - 2 < this.GameMap.GetLength(1))
                {
                    if ((this.GameMap[To.X, To.Y - 2] == 1))
                    {
                        return new Vector2D(To.X, To.Y - 2);
                    }
                }
                ...
                if (To.X + 2 < this.GameMap.GetLength(0) && To.Y < this.GameMap.GetLength(1))
                {
                    if ((this.GameMap[To.X + 2, To.Y] == 1))
                    {
                        return new Vector2D(To.X + 2, To.Y);
                    }
                }
                ...
                if (To.X - 2 < this.GameMap.GetLength(0) && To.Y + 2 < this.GameMap.GetLength(1))
                {
                    if ((this.GameMap[To.X - 2, To.Y + 2] == 1))
                    {
                        return new Vector2D(To.X - 2, To.Y + 2);
                    }
                }
        ...
        internal MovementState GetChasingMovement(int x, int y)
        {
            ...
            return movements[Plus.GetRandomNumber(0, movements.Count)];
        }
        ...
        internal List<Point> getWalkableList()
        {
            List<Point> list = new List<Point>();
            for (int i = 0; i < (this.GameMap.GetUpperBound(1) - 1); i++)
            {
                for (int j = 0; j < (this.GameMap.GetUpperBound(0) - 1); j++)
                {
                    if (((this.StaticModel.DoorX != j) && (this.StaticModel.DoorY != i)) && (this.GameMap[j, i] == 1))
                    {
                        list.Add(new Point(j, i));
                    }
                }
