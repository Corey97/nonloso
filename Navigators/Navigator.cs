...
namespace Plus.HabboHotel.Navigators
{
    ...
    internal class Navigator
    {
        ...
        public void RemovePublicItem(int _id)
        {
            var id = Convert.ToUInt32(_id);
        ...
        internal ServerMessage SerializeNewPublicRooms()
        {
            ...
            message.StartArray();
            foreach (var item in _publicItems.Values.Where(t => t.ParentId == -1))
            {
                message.Clear();
        ...
        {
            var flatcat = Plus.GetGame().GetNavigator().PrivateCategories.OfType<FlatCat>().ToList();
        ...
        internal ServerMessage SerializeFavoriteRooms(GameClient session)
        {
            ...
            foreach (
                var roomData in
                    array
                        .Select(roomId => Plus.GetGame().GetRoomManager().GenerateRoomData(roomId))
                ...
                var roomData in
                    session.GetHabbo()
                        .RecentlyVisitedRooms.Select(
                            current => Plus.GetGame().GetRoomManager().GenerateRoomData(current)))
            {
                roomData.Serialize(serverMessage, false);
                serverMessage.SaveArray();
            }
        ...
        internal ServerMessage SerializeEventListing()
        {
            ...
            serverMessage.AppendString(string.Empty);
            var eventRooms = Plus.GetGame().GetRoomManager().GetEventRooms();
            ...
            using (var queryReactor = Plus.GetDatabaseManager().GetQueryReactor())
            {
                ...
                *
                ...
                foreach (Room Room in Plus.GetGame().GetRoomManager().LoadedRooms.Values.ToList().OrderByDescending(key => key.UserCount))
                {
                    ...
                    if (dictionary.ContainsKey(Room.RoomData.Name)) dictionary[Room.RoomData.Name] += Convert.ToInt32(Room.RoomData.UsersNow);
                    else dictionary.Add(Room.RoomData.Name, Convert.ToInt32(Room.RoomData.UsersNow));
                }
                ...
                case -2:
                    {
                        ...
                        try
                        {
                            var rooms = Plus.GetGame().GetRoomManager().GetVotedRooms();
        ...
        private static int GetNewNavigatorLength(string value)
        {
            switch (value)
            {
                ...
                case "hotel_view":
                case "roomads_view":
                    return Plus.GetGame().GetNavigator().FlatCatsCount + 1;
            }
            return 1;
        }
        ...
        public static ServerMessage SerializeSearchResults(string searchQuery)
        {
            ...
            if (!containsOwner)
            {
                ...
                var activeRooms = Plus.GetGame().GetRoomManager().GetActiveRooms();
                try
                {
            ...
            if (rooms.Count < 50 || containsOwner || containsGroup)
            {
                ...
                if (dTable != null)
                {
                    foreach (var rData in dTable.Rows.Cast<DataRow>().Select(row => Plus.GetGame()
                        .GetRoomManager()
