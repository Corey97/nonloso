...
namespace Plus.HabboHotel.Items.Interactor
{
    class InteractorTrashCans : IFurniInteractor
    {
        ...
        public void OnTrigger(GameClients.GameClient Session, RoomItem Item, int Request, bool HasRights)
        {
            ...
            string foundmsg = "*Non trova nulla di particolarmente interessante.*";

            #region Rummage in bin task

            RoleplayManager.Shout(User.GetClient(), "*Comincia a rovistare nel secchio da buon barbone.*");
            User.ApplyEffect(4);
            ...
            {
                foundmsg = "*Trova ben " + Cash + " euro nel secchio*";
                RoleplayManager.GiveMoney(Session, +Cash);
            }
            else if (foundItem.Equals("[WEED]"))
            {
                foundmsg = "*Trova fortunatamente " + Weed + "g di ottima erba di bidone.*";
            ...
            {
                foundmsg = "*Trova " + Bullets + " pallottole nel bidone*";
            ...
            {
                foundmsg = "*Trova un vecchio vestito malridotto*";
                Session.GetRoleplay().Vests += 1;
                Session.GetRoleplay().SaveQuickStat("vests", "" + Session.GetRoleplay().Vests);
                Session.SendWhisper("Usa :ba per indossare quella cosa...!");
            }
            else if (foundItem.Equals("[PHONEC]"))
            {
                foundmsg = "*Trova " + PhoneCredits + " euro in ricarica telefonica.*";
                RoleplayManager.GiveCredit(Session, +PhoneCredits);
            }
            else if (foundItem.Equals("[HOBO]"))
            {
                //Credits in hand stealer
                HabboHotel.RoomBots.RoomBot NewParamedic = RoleplayManager.MakeQuickBot("[BOT] Marck-Flitzroy", HabboHotel.RoomBots.AIType.MiscBot, User.GetClient().GetHabbo().CurrentRoomId, "hr-676-61.cp-3207-62-1408.ca-3292-110.lg-275-1408.hd-3103-97541.fa-1205-110.ch-255-1331.he-1601-62", Item.X, Item.Y, 0, "M");
                ...
                HabboHotel.RoomBots.RoomBot NewParamedicBank = RoleplayManager.MakeQuickBot("[BOT 2] John-Flitzroy", HabboHotel.RoomBots.AIType.MiscBot, User.GetClient().GetHabbo().CurrentRoomId, "hr-676-61.cp-3207-62-1408.ca-3292-110.lg-275-1408.hd-3103-97541.fa-1205-110.ch-255-1331.he-1601-62", Item.X, Item.Y, 0, "M");
