...
namespace Plus.HabboHotel.RoomBots
{
    class HospitalBot : BotAI
    {
        ...
        internal override void OnUserSay(Rooms.RoomUser User, string Message)
        {
            ...
            if (Message != "cura" && Message != "curami" )
            {
                return;
            }
            ...
            if (Session.GetRoleplay().CurHealth < Session.GetRoleplay().MaxHealth || Session.GetRoleplay().Dead)
            {
                if (!Session.GetRoleplay().Dead)
                {
                    ...
                    {
                        Session.SendWhisper("Sei guarito!");
                        return;
                    }
                    else
                        base.GetRoomUser().Chat(null, "*Comincia a prendersi cura del paziente* " + User.GetClient().GetHabbo().UserName + " vedrai, tra poco starai meglio.", true, 1);
                }
                else
                {
                    ...
                    {
                        base.GetRoomUser().Chat(null, "*Dimette finalmente " + Session.GetHabbo().UserName + " dall'ospedale*", true, 1);
                    ...
                    {
                        Session.SendWhisper("Devi aspettare ancora un minuto prima di poter essere dimesso!");
                        return;
                    }
                }
                ...
                User.GetClient().SendWhisper("Hai la vita piena, non ti servono cure!");
            return;
