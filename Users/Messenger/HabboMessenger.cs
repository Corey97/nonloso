...
namespace Plus.HabboHotel.Users.Messenger
{
    ...
    internal class HabboMessenger
    {
        ...
        internal void SendInstantMessage(uint toId, string message)
        {
            ...
            if (GetClient().GetRoleplay().Phone == 0)
            {
                GetClient().SendWhisperBubble("You do not have a phone! You can buy one at the Phone Store! [Room ID: 5]", 1);
                return;
            }
            if (GetClient().GetHabbo().ActivityPoints < credit)
            {
                GetClient().SendWhisperBubble("You have insufficient phone credit funds! You can buy more at the Phone Store! [Room ID: 5]", 1);
                return;
            }
